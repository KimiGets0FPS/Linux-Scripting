...
def possible_critical_services() -> None:
    ...
    for i in range(len(exclusion)):
        ...
        run_commands([f"{_command} install {exclusion[i]} -y", f"{_command} upgrade {exclusion[i]}"],
...
def password_policy_config() -> None:
    ...
    run_command(
        "sed -i '/pam_cracklib.so/c\password required pam_cracklib.so try_first_pass retry=3 minlen=10 dcredit=-1 ucredit=-1 lcredit=-1 ocredit=-1' /etc/pam.d/common-password")

    # Configure account lockout policy
    run_command(
        "sed -i '/^auth.*required.*pam_unix.so/i auth required pam_tally2.so onerr=fail audit silent deny=5 unlock_time=900' /etc/pam.d/common-auth")

    # Configure password history/reuse
    run_command(
        "sed -i '/^password.*sufficient.*pam_unix.so/c\password sufficient pam_unix.so remember=5' /etc/pam.d/common-password")

    # Configure SHA-512 password hashing
    run_command(
        "sed -i '/^password.*pam_unix.so/c\password [success=1 default=ignore] pam_unix.so sha512' /etc/pam.d/common-password")
